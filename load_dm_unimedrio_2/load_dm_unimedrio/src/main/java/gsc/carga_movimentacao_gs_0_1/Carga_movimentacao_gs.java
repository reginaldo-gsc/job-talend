...
package gsc.carga_movimentacao_gs_0_1;
...
/**
 * Job: Carga_movimentacao_gs Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 7.3.1.20200219_1130
 * @status 
 */
public class Carga_movimentacao_gs implements TalendJob {
...
protected static void logIgnoredError(String message, Throwable cause) {
	...
	private final String jobName = "Carga_movimentacao_gs";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Carga_movimentacao_gs.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Carga_movimentacao_gs.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
    final static byte[] commonByteArrayLock_GSC_Carga_movimentacao_gs = new byte[0];
    static byte[] commonByteArray_GSC_Carga_movimentacao_gs = new byte[0];
			    ...
			    public String sts_nome;

				public String getSts_nome () {
					return this.sts_nome;
				}
				
			    public String status_paciente;

				public String getStatus_paciente () {
					return this.status_paciente;
				}
				
			    public java.util.Date sts_sys_dt_alt;

				public java.util.Date getSts_sys_dt_alt () {
					return this.sts_sys_dt_alt;
				}
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_GSC_Carga_movimentacao_gs.length) {
				if(length < 1024 && commonByteArray_GSC_Carga_movimentacao_gs.length == 0) {
   					commonByteArray_GSC_Carga_movimentacao_gs = new byte[1024];
				} else {
   					commonByteArray_GSC_Carga_movimentacao_gs = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_GSC_Carga_movimentacao_gs, 0, length);
			strReturn = new String(commonByteArray_GSC_Carga_movimentacao_gs, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_GSC_Carga_movimentacao_gs) {
        	...
        	try {
        		...
        		int length = 0;
					...
					this.sts_nome = readString(dis);
					
					this.status_paciente = readString(dis);
					
					this.sts_sys_dt_alt = readDate(dis);
    ...
    public void writeData(ObjectOutputStream dos) {
        try {
					...
					// Character
						...
						if(this.pac_sexo == null) {
			                dos.writeByte(-1);
						} else {
               				dos.writeByte(0);
           			    	dos.writeChar(this.pac_sexo);
		            	}
					...
					// String
						...
						writeString(this.sts_nome,dos);
					
					// String
				
						writeString(this.status_paciente,dos);
					
					// java.util.Date
				
						writeDate(this.sts_sys_dt_alt,dos);
    ...
    public String toString() {
		...
		sb.append(",pac_sexo="+String.valueOf(pac_sexo));
		...
		sb.append(",sts_nome="+sts_nome);
		sb.append(",status_paciente="+status_paciente);
		sb.append(",sts_sys_dt_alt="+String.valueOf(sts_sys_dt_alt));
...
String dbUser_tSQLDWHOutput_1 = null;
	...
	final String decryptedPassword_tSQLDWHOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:9Byjnj2hCxo5pmSUnSmyUuAu9qGsHsQpMaBxU1q7DPwX9z2F0FJu");
...
if(dbschema_tSQLDWHOutput_1 == null || dbschema_tSQLDWHOutput_1.trim().length() == 0) {
    tableName_tSQLDWHOutput_1 = "tb_movimentacao_gs";
} else {
    tableName_tSQLDWHOutput_1 = dbschema_tSQLDWHOutput_1 + "].[" + "tb_movimentacao_gs";
}
	int count_tSQLDWHOutput_1=0;
        ...
        String insert_tSQLDWHOutput_1 = "INSERT INTO [" + tableName_tSQLDWHOutput_1 + "] ([pac_codigo],[pac_nome],[cnv_nome],[pac_dt_nasc],[pac_sexo],[pac_end],[pac_num],[pac_bairro],[pac_cid],[pac_UF],[pac_cep],[nivel_nome],[sts_nome],[status_paciente],[sts_sys_dt_alt],[usu_login],[dt_atlz]) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
 ...
 */
	...
	final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:YnC4Nyj61VQ2GmH1ZuWmoJMIj4CYB6v5kuk+12NUiDVnYwPdR7M=");
...
+"ys_dt_alt,\na.usu_login,\nCONCAT (date_format (case when b.pac_sys_dt_alt = '0000-00-00' then '1900-01-01' else b.pac_sy"
+"s_dt_alt end, \"%Y-%m-%d\"),' ', b.pac_sys_hr_alt,':00') as dt_atlz\nfrom gsc_status_mov a\ninner join gsc_paciente b on"
+" b.pac_codigo = a.pac_codigo \ninner join gsc_convenio c on c.cnv_codigo = b.cnv_codigo \ninner join gsc_nivel_atencao_3"
+"60 d on d.nivel_codigo = b.pro_codigo\ninner join gsc_status e on e.sts_codigo = a.sts_codigo \ninner join gsc_status f "
+"on f.sts_codigo = b.sts_codigo \nORDER BY a.sts_seq DESC\n)x";
        	...
        	row1.cnv_nome = routines.system.JDBCUtil.getString(rs_tDBInput_1, 3, false);
				...
				if(rs_tDBInput_1.getString(4) != null) {
					...
					if (!("0000-00-00").equals(dateString_tDBInput_1) && !("0000-00-00 00:00:00").equals(dateString_tDBInput_1)) {
						row1.pac_dt_nasc = rs_tDBInput_1.getTimestamp(4);
					} else {
                    ...
                    if(tmpContent_tDBInput_1 != null && tmpContent_tDBInput_1.length() > 0) {			  	
                        row1.pac_sexo = tmpContent_tDBInput_1.charAt(0);			  		
                    } else {			  				  	    
        	...
        	row1.sts_nome = routines.system.JDBCUtil.getString(rs_tDBInput_1, 13, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 14) {
								row1.status_paciente = null;
							} else {
	                         		
        	row1.status_paciente = routines.system.JDBCUtil.getString(rs_tDBInput_1, 14, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 15) {
								row1.sts_sys_dt_alt = null;
							} else {
										
				if(rs_tDBInput_1.getString(15) != null) {
					String dateString_tDBInput_1 = rs_tDBInput_1.getString(15);
					if (!("0000-00-00").equals(dateString_tDBInput_1) && !("0000-00-00 00:00:00").equals(dateString_tDBInput_1)) {
						row1.sts_sys_dt_alt = rs_tDBInput_1.getTimestamp(15);
					} else {
						row1.sts_sys_dt_alt = (java.util.Date) year0_tDBInput_1.clone();
					}
				} else {
					row1.sts_sys_dt_alt =  null;
				}
        	...
        	row1.usu_login = routines.system.JDBCUtil.getString(rs_tDBInput_1, 16, false);
		                    }
...
pstmt_tSQLDWHOutput_1.setNull(3, java.sql.Types.VARCHAR);
                    ...
                    if(row2.pac_dt_nasc != null) {
pstmt_tSQLDWHOutput_1.setTimestamp(4, new java.sql.Timestamp(row2.pac_dt_nasc.getTime()));
} else {
pstmt_tSQLDWHOutput_1.setNull(4, java.sql.Types.DATE);
}

                    if(row2.pac_sexo == null) {
pstmt_tSQLDWHOutput_1.setNull(5, java.sql.Types.CHAR);
} else {if(row2.pac_sexo == null) {
pstmt_tSQLDWHOutput_1.setNull(5, java.sql.Types.CHAR);
} else if(row2.pac_sexo == ' '){
pstmt_tSQLDWHOutput_1.setString(5, "");
} else {
pstmt_tSQLDWHOutput_1.setString(5, String.valueOf(row2.pac_sexo));
}}

                    if(row2.pac_end == null) {
pstmt_tSQLDWHOutput_1.setNull(6, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(6, row2.pac_end);
}

                    if(row2.pac_num == null) {
pstmt_tSQLDWHOutput_1.setNull(7, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(7, row2.pac_num);
}

                    if(row2.pac_bairro == null) {
pstmt_tSQLDWHOutput_1.setNull(8, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(8, row2.pac_bairro);
}

                    if(row2.pac_cid == null) {
pstmt_tSQLDWHOutput_1.setNull(9, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(9, row2.pac_cid);
}

                    if(row2.pac_UF == null) {
pstmt_tSQLDWHOutput_1.setNull(10, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(10, row2.pac_UF);
}

                    if(row2.pac_cep == null) {
pstmt_tSQLDWHOutput_1.setNull(11, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(11, row2.pac_cep);
}

                    if(row2.nivel_nome == null) {
pstmt_tSQLDWHOutput_1.setNull(12, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(12, row2.nivel_nome);
}

                    if(row2.sts_nome == null) {
pstmt_tSQLDWHOutput_1.setNull(13, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(13, row2.sts_nome);
}

                    if(row2.status_paciente == null) {
pstmt_tSQLDWHOutput_1.setNull(14, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(14, row2.status_paciente);
}

                    if(row2.sts_sys_dt_alt != null) {
pstmt_tSQLDWHOutput_1.setTimestamp(15, new java.sql.Timestamp(row2.sts_sys_dt_alt.getTime()));
} else {
...
} else {pstmt_tSQLDWHOutput_1.setString(16, row2.usu_login);
}
 ...
 */
    ...
    public static void main(String[] args){
        final Carga_movimentacao_gs Carga_movimentacao_gsClass = new Carga_movimentacao_gs();

        int exitCode = Carga_movimentacao_gsClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Carga_movimentacao_gs.class.getClassLoader().getResourceAsStream("gsc/carga_movimentacao_gs_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = Carga_movimentacao_gs.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Carga_movimentacao_gs");
        }
...
/************************************************************************************************
 *     69426 characters generated by Talend Open Studio for Data Integration 
