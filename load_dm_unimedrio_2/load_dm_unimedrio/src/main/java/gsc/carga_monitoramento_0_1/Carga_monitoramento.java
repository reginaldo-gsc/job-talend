...
package gsc.carga_monitoramento_0_1;
...
/**
 * Job: Carga_monitoramento Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 7.3.1.20200219_1130
 * @status 
 */
public class Carga_monitoramento implements TalendJob {
...
protected static void logIgnoredError(String message, Throwable cause) {
	...
	private final String jobName = "Carga_monitoramento";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Carga_monitoramento.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Carga_monitoramento.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class outStruct implements routines.system.IPersistableRow<outStruct> {
    final static byte[] commonByteArrayLock_GSC_Carga_monitoramento = new byte[0];
    static byte[] commonByteArray_GSC_Carga_monitoramento = new byte[0];
			    ...
			    public String pac_matricula;

				public String getPac_matricula () {
					return this.pac_matricula;
				}
			    ...
			    public java.util.Date age_dt_ini;

				public java.util.Date getAge_dt_ini () {
					return this.age_dt_ini;
				}
				
			    public String prc_nome;

				public String getPrc_nome () {
					return this.prc_nome;
				}
			    ...
			    public String age_obsrv;

				public String getAge_obsrv () {
					return this.age_obsrv;
				}
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_GSC_Carga_monitoramento.length) {
				if(length < 1024 && commonByteArray_GSC_Carga_monitoramento.length == 0) {
   					commonByteArray_GSC_Carga_monitoramento = new byte[1024];
				} else {
   					commonByteArray_GSC_Carga_monitoramento = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_GSC_Carga_monitoramento, 0, length);
			strReturn = new String(commonByteArray_GSC_Carga_monitoramento, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_GSC_Carga_monitoramento) {
        	...
        	try {
        		...
        		int length = 0;
					...
					this.pac_matricula = readString(dis);
					...
					this.age_dt_ini = readDate(dis);
					
					this.prc_nome = readString(dis);
					
					this.status = readString(dis);
					
					this.dt_atlz = readDate(dis);
					
					this.age_obsrv = readString(dis);
    ...
    public void writeData(ObjectOutputStream dos) {
        try {
					...
					// String
						...
						writeString(this.pac_matricula,dos);
					...
					// java.util.Date
						...
						writeDate(this.age_dt_ini,dos);
					
					// String
				
						writeString(this.prc_nome,dos);
					...
					// String
						...
						writeString(this.age_obsrv,dos);
    ...
    public String toString() {
		...
		sb.append(",pac_matricula="+pac_matricula);
		sb.append(",tipo="+tipo);
		...
		sb.append(",age_dt_ini="+String.valueOf(age_dt_ini));
		sb.append(",prc_nome="+prc_nome);
		sb.append(",status="+status);
		sb.append(",dt_atlz="+String.valueOf(dt_atlz));
		sb.append(",age_obsrv="+age_obsrv);
	    sb.append("]");
...
String dbUser_tSQLDWHOutput_1 = null;
	...
	final String decryptedPassword_tSQLDWHOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:UrtjjnzZvCnkkqqS0c8xYnhxXnUoB6Pw68lBR2A2bp/i2Ulhqr5z");
...
if(dbschema_tSQLDWHOutput_1 == null || dbschema_tSQLDWHOutput_1.trim().length() == 0) {
    tableName_tSQLDWHOutput_1 = "tb_monitoramento";
} else {
    tableName_tSQLDWHOutput_1 = dbschema_tSQLDWHOutput_1 + "].[" + "tb_monitoramento";
}
	int count_tSQLDWHOutput_1=0;
        ...
        String insert_tSQLDWHOutput_1 = "INSERT INTO [" + tableName_tSQLDWHOutput_1 + "] ([usu_login],[pac_matricula],[tipo],[pac_codigo],[pac_nome],[cnv_nome],[nivel_nome],[pac_end],[pac_num],[pac_bairro],[pac_cid],[pac_UF],[pac_cep],[age_dt_ini],[prc_nome],[status],[dt_atlz],[age_obsrv]) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
 ...
 */
	...
	final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:/OSytqqF9iycaxXWsv8SclPYcsmX12lVHzZ+Lg+Tr9C1s6LkocI=");
		    ...
		    String dbquery_tDBInput_1 = "select\na.usu_login\n,b.pac_matricula\n,case when a.age_tipo = 1 then 'Telefonico' when a.age_tipo = 2 then 'Presencial"
+"' else '' end as 'tipo'\n,b.pac_codigo\n,b.pac_nome\n,c.cnv_nome\n,d.nivel_nome\n,b.pac_end\n,b.pac_num\n,b.pac_bairro\n"
+",b.pac_cid\n,b.`pac_UF`\n,b.pac_cep\n,date_format (case when a.age_dt_ini = '0000-00-00' then '1900-01-01' else a.age_dt"
+"_ini end, \"%Y-%m-%d\") as age_dt_ini\n,e.prc_nome\n,case\n    when a.age_status = 1 then  'Programado'\n    when a.age_"
+"status = 2 then  'Agendado'\n    when a.age_status = 3 then  'Realizado'\n    when a.age_status = 4 then  'Não Realizado"
+"'\n    when a.age_status = 5 then  'Realizado sem exito'\n    when a.age_status = 6 then  'Substituicao de evento'\n    "
+"else '' end as 'status'\n    ,CONCAT(date_format (case when b.pac_sys_dt_alt = '0000-00-00' then '1900-01-01' else b.pac"
+"_sys_dt_alt end, \"%Y-%m-%d\"),' ', b.pac_sys_hr_alt,':00') as dt_atlz   \n    ,a.age_obsrv\nfrom gss_agenda a\ninner jo"
+"in gsc_paciente b on a.pac_codigo = b.pac_codigo\ninner join gsc_convenio c on c.cnv_codigo = b.cnv_codigo\ninner join g"
+"sc_nivel_atencao_360 d on d.nivel_codigo = b.pro_codigo\nleft join gsc_procedimento_360 e on a.prc_codigo = e.prc_codigo"
+"";
        	...
        	row1.usu_login = routines.system.JDBCUtil.getString(rs_tDBInput_1, 1, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 2) {
								row1.pac_matricula = null;
							} else {
	                         		
        	row1.pac_matricula = routines.system.JDBCUtil.getString(rs_tDBInput_1, 2, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 3) {
								row1.tipo = null;
							} else {
	                         		
        	row1.tipo = routines.system.JDBCUtil.getString(rs_tDBInput_1, 3, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 4) {
								row1.pac_codigo = null;
							} else {
		                          
            row1.pac_codigo = rs_tDBInput_1.getInt(4);
        	...
        	row1.pac_nome = routines.system.JDBCUtil.getString(rs_tDBInput_1, 5, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 6) {
								row1.cnv_nome = null;
							} else {
	                         		
        	row1.cnv_nome = routines.system.JDBCUtil.getString(rs_tDBInput_1, 6, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 7) {
								row1.nivel_nome = null;
							} else {
	                         		
        	row1.nivel_nome = routines.system.JDBCUtil.getString(rs_tDBInput_1, 7, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 8) {
								row1.pac_end = null;
							} else {
	                         		
        	row1.pac_end = routines.system.JDBCUtil.getString(rs_tDBInput_1, 8, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 9) {
								row1.pac_num = null;
							} else {
	                         		
        	row1.pac_num = routines.system.JDBCUtil.getString(rs_tDBInput_1, 9, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 10) {
								row1.pac_bairro = null;
							} else {
	                         		
        	row1.pac_bairro = routines.system.JDBCUtil.getString(rs_tDBInput_1, 10, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 11) {
								row1.pac_cid = null;
							} else {
	                         		
        	row1.pac_cid = routines.system.JDBCUtil.getString(rs_tDBInput_1, 11, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 12) {
								row1.pac_UF = null;
							} else {
	                         		
        	row1.pac_UF = routines.system.JDBCUtil.getString(rs_tDBInput_1, 12, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 13) {
								row1.pac_cep = null;
							} else {
	                         		
        	row1.pac_cep = routines.system.JDBCUtil.getString(rs_tDBInput_1, 13, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 14) {
								row1.age_dt_ini = null;
							} else {
				...
				if(rs_tDBInput_1.getString(14) != null) {
					...
					if (!("0000-00-00").equals(dateString_tDBInput_1) && !("0000-00-00 00:00:00").equals(dateString_tDBInput_1)) {
						row1.age_dt_ini = rs_tDBInput_1.getTimestamp(14);
					} else {
						row1.age_dt_ini = (java.util.Date) year0_tDBInput_1.clone();
					}
				} else {
					row1.age_dt_ini =  null;
				}
        	...
        	row1.prc_nome = routines.system.JDBCUtil.getString(rs_tDBInput_1, 15, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 16) {
								row1.status = null;
							} else {
	                         		
        	row1.status = routines.system.JDBCUtil.getString(rs_tDBInput_1, 16, false);
		                    }
        	...
        	row1.age_obsrv = routines.system.JDBCUtil.getString(rs_tDBInput_1, 18, false);
		                    }
...
out_tmp.pac_matricula = row1.pac_matricula;
out_tmp.tipo = row1.tipo;
...
out_tmp.age_dt_ini = row1.age_dt_ini;
out_tmp.prc_nome = row1.prc_nome;
out_tmp.status = row1.status;
out_tmp.dt_atlz = row1.dt_atlz;
out_tmp.age_obsrv = row1.age_obsrv ;
out = out_tmp;
// ###############################
...
} else {pstmt_tSQLDWHOutput_1.setString(1, out.usu_login);
}

                    if(out.pac_matricula == null) {
pstmt_tSQLDWHOutput_1.setNull(2, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(2, out.pac_matricula);
}

                    if(out.tipo == null) {
pstmt_tSQLDWHOutput_1.setNull(3, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(3, out.tipo);
}

                    if(out.pac_codigo == null) {
pstmt_tSQLDWHOutput_1.setNull(4, java.sql.Types.INTEGER);
} else {pstmt_tSQLDWHOutput_1.setInt(4, out.pac_codigo);
}

                    if(out.pac_nome == null) {
pstmt_tSQLDWHOutput_1.setNull(5, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(5, out.pac_nome);
}

                    if(out.cnv_nome == null) {
pstmt_tSQLDWHOutput_1.setNull(6, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(6, out.cnv_nome);
}

                    if(out.nivel_nome == null) {
pstmt_tSQLDWHOutput_1.setNull(7, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(7, out.nivel_nome);
}

                    if(out.pac_end == null) {
pstmt_tSQLDWHOutput_1.setNull(8, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(8, out.pac_end);
}

                    if(out.pac_num == null) {
pstmt_tSQLDWHOutput_1.setNull(9, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(9, out.pac_num);
}

                    if(out.pac_bairro == null) {
pstmt_tSQLDWHOutput_1.setNull(10, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(10, out.pac_bairro);
}

                    if(out.pac_cid == null) {
pstmt_tSQLDWHOutput_1.setNull(11, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(11, out.pac_cid);
}

                    if(out.pac_UF == null) {
pstmt_tSQLDWHOutput_1.setNull(12, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(12, out.pac_UF);
}

                    if(out.pac_cep == null) {
pstmt_tSQLDWHOutput_1.setNull(13, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(13, out.pac_cep);
}

                    if(out.age_dt_ini != null) {
pstmt_tSQLDWHOutput_1.setTimestamp(14, new java.sql.Timestamp(out.age_dt_ini.getTime()));
} else {
pstmt_tSQLDWHOutput_1.setNull(14, java.sql.Types.DATE);
}

                    if(out.prc_nome == null) {
pstmt_tSQLDWHOutput_1.setNull(15, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(15, out.prc_nome);
}

                    if(out.status == null) {
pstmt_tSQLDWHOutput_1.setNull(16, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(16, out.status);
}

                    if(out.dt_atlz != null) {
pstmt_tSQLDWHOutput_1.setTimestamp(17, new java.sql.Timestamp(out.dt_atlz.getTime()));
} else {
pstmt_tSQLDWHOutput_1.setNull(17, java.sql.Types.DATE);
}

                    if(out.age_obsrv == null) {
pstmt_tSQLDWHOutput_1.setNull(18, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(18, out.age_obsrv);
}
 ...
 */
    ...
    public static void main(String[] args){
        final Carga_monitoramento Carga_monitoramentoClass = new Carga_monitoramento();

        int exitCode = Carga_monitoramentoClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Carga_monitoramento.class.getClassLoader().getResourceAsStream("gsc/carga_monitoramento_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = Carga_monitoramento.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Carga_monitoramento");
        }
...
/************************************************************************************************
 *     70361 characters generated by Talend Open Studio for Data Integration 
