...
/**
 * Job: Carga_pah Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 7.3.1.20200219_1130
 * @status 
 */
public class Carga_pah implements TalendJob {
...
protected static void logIgnoredError(String message, Throwable cause) {
	...
	private final String jobName = "Carga_pah";
		...
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Carga_pah.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
    final static byte[] commonByteArrayLock_GSC_Carga_pah = new byte[0];
    static byte[] commonByteArray_GSC_Carga_pah = new byte[0];
			    ...
			    public java.util.Date age_pah_dt_ini;

				public java.util.Date getAge_pah_dt_ini () {
					return this.age_pah_dt_ini;
				}
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_GSC_Carga_pah.length) {
				if(length < 1024 && commonByteArray_GSC_Carga_pah.length == 0) {
   					commonByteArray_GSC_Carga_pah = new byte[1024];
				} else {
   					commonByteArray_GSC_Carga_pah = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_GSC_Carga_pah, 0, length);
			strReturn = new String(commonByteArray_GSC_Carga_pah, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_GSC_Carga_pah) {
        	...
        	try {
        		...
        		int length = 0;
					...
					this.age_pah_dt_ini = readDate(dis);
					
					this.procedimento = readString(dis);
    ...
    public void writeData(ObjectOutputStream dos) {
        try {
					...
					// java.util.Date
						...
						writeDate(this.age_pah_dt_ini,dos);
					
					// String
				
						writeString(this.procedimento,dos);
    ...
    public String toString() {
		...
		sb.append(",age_pah_dt_ini="+String.valueOf(age_pah_dt_ini));
...
String dbUser_tSQLDWHOutput_1 = null;
	...
	final String decryptedPassword_tSQLDWHOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:ToHJtX25s9kJegifkOoY6VgqLGtkuu8/YRuhXzAicjdY2fwtzqeC");
...
if(dbschema_tSQLDWHOutput_1 == null || dbschema_tSQLDWHOutput_1.trim().length() == 0) {
    tableName_tSQLDWHOutput_1 = "tb_pah";
} else {
    tableName_tSQLDWHOutput_1 = dbschema_tSQLDWHOutput_1 + "].[" + "tb_pah";
}
	int count_tSQLDWHOutput_1=0;
        ...
        String insert_tSQLDWHOutput_1 = "INSERT INTO [" + tableName_tSQLDWHOutput_1 + "] ([usu_login],[pac_matricula],[tipo],[pac_codigo],[pac_nome],[cnv_nome],[nivel_nome],[pac_end],[pac_num],[pac_bairro],[pac_cid],[pac_UF],[pac_cep],[age_pah_dt_ini],[procedimento],[status],[dt_atlz]) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
 ...
 */
	...
	final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:C0+7t+UJWMONHk4RHiJpNfQ8diqImhOG1eFvKov73aAjW09S0F4=");
		    ...
		    String dbquery_tDBInput_1 = "select\na.usu_login\n,b.pac_matricula\n,case\n    when a.age_pah_tipo = 1 then 'Telefonico'\n    when a.age_pah_tipo = "
+"2 then 'Presencial'\n    else '' end as 'tipo'\n,b.pac_codigo\n,b.pac_nome\n,c.cnv_nome\n,d.nivel_nome\n,b.pac_end\n,b.p"
+"ac_num\n,b.pac_bairro\n,b.pac_cid\n,b.`pac_UF`\n,b.pac_cep\n,a.age_pah_dt_ini\n,case \n	when a.age_tipo_acompanhamento ="
+" 1 then 'Internação'\n	else 'PAH' end as 'procedimento'\n,case\n    when a.age_pah_status = 1 then  'Programado'\n    wh"
+"en a.age_pah_status = 2 then  'Agendado'\n    when a.age_pah_status = 3 then  'Realizado'\n    when a.age_pah_status = 4"
+" then  'Não Realizado'\n    when a.age_pah_status = 5 then  'Realizado sem exito'\n    when a.age_pah_status = 6 then  '"
+"Substituicao de evento'\n    else '' end as 'status'\n    ,CONCAT (b.pac_sys_dt_alt,' ', b.pac_sys_hr_alt,':00') as dt_a"
+"tlz\nfrom gss_agenda_pah a\ninner join gsc_paciente b on a.pac_codigo = b.pac_codigo\ninner join gsc_convenio c on c.cnv"
+"_codigo = b.cnv_codigo\ninner join gsc_nivel_atencao_360 d on d.nivel_codigo = b.pro_codigo\nwhere b.cnv_codigo = 114;";
        	...
        	row1.pac_cep = routines.system.JDBCUtil.getString(rs_tDBInput_1, 13, false);
				...
				if(rs_tDBInput_1.getString(14) != null) {
					...
					if (!("0000-00-00").equals(dateString_tDBInput_1) && !("0000-00-00 00:00:00").equals(dateString_tDBInput_1)) {
						row1.age_pah_dt_ini = rs_tDBInput_1.getTimestamp(14);
					} else {
						row1.age_pah_dt_ini = (java.util.Date) year0_tDBInput_1.clone();
					}
				} else {
					row1.age_pah_dt_ini =  null;
				}
							...
							if(colQtyInRs_tDBInput_1 < 15) {
								row1.procedimento = null;
							} else {
	                         		
        	row1.procedimento = routines.system.JDBCUtil.getString(rs_tDBInput_1, 15, false);
		                    }
...
} else {pstmt_tSQLDWHOutput_1.setString(1, row2.usu_login);
}

                    if(row2.pac_matricula == null) {
pstmt_tSQLDWHOutput_1.setNull(2, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(2, row2.pac_matricula);
}

                    if(row2.tipo == null) {
pstmt_tSQLDWHOutput_1.setNull(3, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(3, row2.tipo);
}

                    if(row2.pac_codigo == null) {
pstmt_tSQLDWHOutput_1.setNull(4, java.sql.Types.INTEGER);
} else {pstmt_tSQLDWHOutput_1.setInt(4, row2.pac_codigo);
}

                    if(row2.pac_nome == null) {
pstmt_tSQLDWHOutput_1.setNull(5, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(5, row2.pac_nome);
}

                    if(row2.cnv_nome == null) {
pstmt_tSQLDWHOutput_1.setNull(6, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(6, row2.cnv_nome);
}

                    if(row2.nivel_nome == null) {
pstmt_tSQLDWHOutput_1.setNull(7, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(7, row2.nivel_nome);
}

                    if(row2.pac_end == null) {
pstmt_tSQLDWHOutput_1.setNull(8, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(8, row2.pac_end);
}

                    if(row2.pac_num == null) {
pstmt_tSQLDWHOutput_1.setNull(9, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(9, row2.pac_num);
}

                    if(row2.pac_bairro == null) {
pstmt_tSQLDWHOutput_1.setNull(10, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(10, row2.pac_bairro);
}

                    if(row2.pac_cid == null) {
pstmt_tSQLDWHOutput_1.setNull(11, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(11, row2.pac_cid);
}

                    if(row2.pac_UF == null) {
pstmt_tSQLDWHOutput_1.setNull(12, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(12, row2.pac_UF);
}

                    if(row2.pac_cep == null) {
pstmt_tSQLDWHOutput_1.setNull(13, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(13, row2.pac_cep);
}

                    if(row2.age_pah_dt_ini != null) {
pstmt_tSQLDWHOutput_1.setTimestamp(14, new java.sql.Timestamp(row2.age_pah_dt_ini.getTime()));
} else {
pstmt_tSQLDWHOutput_1.setNull(14, java.sql.Types.DATE);
}

                    if(row2.procedimento == null) {
pstmt_tSQLDWHOutput_1.setNull(15, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(15, row2.procedimento);
}

                    if(row2.status == null) {
pstmt_tSQLDWHOutput_1.setNull(16, java.sql.Types.VARCHAR);
} else {pstmt_tSQLDWHOutput_1.setString(16, row2.status);
}

                    if(row2.dt_atlz != null) {
pstmt_tSQLDWHOutput_1.setTimestamp(17, new java.sql.Timestamp(row2.dt_atlz.getTime()));
} else {
 ...
 */
    ...
    public static void main(String[] args){
        final Carga_pah Carga_pahClass = new Carga_pah();

        int exitCode = Carga_pahClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Carga_pah.class.getClassLoader().getResourceAsStream("gsc/carga_pah_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = Carga_pah.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Carga_pah");
        }
...
/************************************************************************************************
 *     67135 characters generated by Talend Open Studio for Data Integration 
